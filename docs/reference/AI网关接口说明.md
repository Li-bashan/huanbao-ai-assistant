...
AI Gateway 是前端与模型能力、权限、会话和审计之间的服务端入口。当前智能问数由 Gateway 转发到独立 `huanbao-dataquery:8089` 执行；制度、办公和统一 Master 仍经 Gateway 兼容 Dify。流程助手仍只走前端动作桥接。当前去 Dify 化的目标架构和迁移顺序见 [去 Dify 化智能助手整体建设方案](../去Dify化智能助手整体建设方案.md)。
...
| /api/ai/master/chat | POST SSE | 制度和办公统一入口 | 迁移期间兼容 Dify；去 Dify 化待施工 |
...
| /api/ai/data-query/chat | POST SSE | 问数身份、授权、会话、SSE、审计及独立服务代理 | 当前生产链路 |
...
首轮可以不传 conversationId，Gateway 生成用户专属 opaque 会话句柄。问数服务内部会话标识只保存在服务端，不返回浏览器。后续请求校验租户、用户、模式和句柄归属；制度和办公的 Dify conversation id 同样只保存在服务端。
...
Master、Policy、Office 的 Dify Base URL 和 Key 只配置在 Gateway 服务端；Data Query 当前只配置独立问数服务地址，不需要 Dify Key。仓库不记录真实 Key，也不把 Dify API 地址作为浏览器直接调用入口。
...
- 对制度、办公和 Master 兼容链路，不能仅凭 Gateway 在线返回 UP 推出 Dify Key 绑定了哪个 App。
- 不能仅凭 Dify 页面可达推出制度引用、办公生成或 Kingbase 查询成功。
- 当前问数不再经过 Dify；必须以独立服务的权限、SQL、语义、数据质量和证据回归为准。
