...
const formatPercent = (value) => `${formatValue(value)}%`
const comparisonFacts = computed(() => {
  const rowFacts = rows.value.filter((row) =>
    hasValue(row.value) || hasValue(row.yearOverYearPercent) || hasValue(row.monthOverMonthPercent),
  )
  if (rowFacts.length) return rowFacts
  return metrics.value.filter((metric) =>
    hasValue(metric.yearOverYearPercent) || hasValue(metric.monthOverMonthPercent),
  )
})
const factLabel = (fact) => fact.organization || fact.companyName || fact.subject || fact.label || indicatorName.value
...
<template>
  <article class="data-query-result data-query-analysis-card data-query-comparison-view">
    <header class="data-query-analysis-header">
      <div>
        <span class="data-query-analysis-kicker">对比</span>
        <h3>{{ content.title || `${indicatorName}对比` }}</h3>
      </div>
    ...
    <section v-if="comparisonFacts.length" class="data-query-comparison-facts" aria-label="对比事实">
      ...
      <div v-for="(fact, index) in comparisonFacts" :key="`${factLabel(fact)}-${index}`" class="data-query-comparison-fact">
        ...
        <span v-if="hasValue(fact.value)">当前值 {{ formatValue(fact.value) }}</span>
        <span v-if="hasValue(fact.yearOverYearPercent)">同比 {{ formatPercent(fact.yearOverYearPercent) }}</span>
...
.data-query-comparison-facts-title {
  color: #536b82;
  font-size: 11px;
  font-weight: 800;
}

.data-query-comparison-fact {
  display: flex;
  flex-wrap: wrap;
...
.data-query-comparison-fact strong {
  min-width: 120px;
  color: #294b6d;
}
</style>
